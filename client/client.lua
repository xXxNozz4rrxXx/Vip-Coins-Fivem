...
AddEventHandler('trew_hud_ui:setInfo', function(info)
	...
	SendNUIMessage({ action = 'setMoney', id = 'vipcoin', value = info['vipcoin'] })
...
RegisterCommand('toggleui', function()
	if not toggleui then
		...
		SendNUIMessage({ action = 'element', task = 'disable', value = 'vipcoin' })
	else
		...
		if (Config.ui.showSpecialMoney == true) then
			SendNUIMessage({ action = 'element', task = 'enable', value = 'vipcoin' })
		end
	end
